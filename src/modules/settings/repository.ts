...
export async function findSettingsByUserId(db: DB, userId: string) {
	return db.select().from(settingsInPrivate).where(eq(settingsInPrivate.userId, userId)).then(takeUniqueOrThrow);
}
...
export async function updateSettings(db: DB, userId: string, settings: SettingsRequest) {
	return db
