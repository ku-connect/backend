...
async function cleanupBeforeSeed() {
	await db.transaction(async (tx) => {
		await tx.delete(profileInPrivate).where(
			inArray(
				profileInPrivate.userId,
				users.map((user) => user.id)
			)
		);
		await tx.delete(userInterestInPrivate).where(
			inArray(
				userInterestInPrivate.userId,
				users.map((user) => user.id)
			)
		);
		await tx.delete(settingsInPrivate).where(
			inArray(
				settingsInPrivate.userId,
				users.map((user) => user.id)
			)
		);
		await tx.delete(usersInAuth).where(
			inArray(
				usersInAuth.id,
				users.map((user) => user.id)
			)
